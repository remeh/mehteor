...
void AudioSource::play(Sound* sound, bool loop) 
{
    ...
    alBufferData(buffId, sound->getFormat(), &(sound->getData()[0]), static_cast<ALsizei>(sound->getData().size()), sound->getFrequency());

    // attach the buffer to the source
    alSourcei(srcId, AL_BUFFER, buffId);

    // compute in how many milli-seconds this Sound will be fully played.
    endTime = System::currentTime() + sound->getDuration() + 15;
